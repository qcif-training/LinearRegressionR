...
and varies between -1 and 1, where 1 indicates a perfect positive linear relationship and -1 indicates a perfect negative linear relationship. Here, 
$$\bar{x}$$ is the mean of all values of x. 
...
The p-value resulting from this test is significant, so we can reject the null hypothesis that the data is normally distributed 
...
However, we will first separately analyse the 'ca' variable, the number of major vessels coloured by fluoroscopy, as this is 
an ordinal variable, which requires we use Kendall's Tau to measure correlation.
