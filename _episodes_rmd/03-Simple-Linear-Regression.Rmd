...
Regression analysis is used to describe the relationship between a single dependent variable _Y_ (also called the outcome) and one or more independent variables _X<sub>1</sub>, X<sub>2</sub>, ..., X<sub>p</sub>_ (also called the predictors).
...
In simple linear regression, the aim is to predict the dependent variable _Y_ based on a given value of the independent variable _X_ using a linear relationship. This relationship is described by the following equation:
...
_e_ represents an additive residual term (error) that accounts for random statistical noise or the effect of unobserved independent variables not included in the model.
