...
If we have $p$ independent variables, then our model is
...
![RStudio layout](fig/04-correlations.png)
...
![RStudio layout](fig/04-model-summary.png)

The estimated coefficients of the model is shown in the figure below.

![RStudio layout](fig/04-coefficients.png)
...
![RStudio layout](fig/04-coefficients-ci.png)
...
![RStudio layout](fig/04-examples.png)
...
![RStudio layout](fig/04-flowchart.png)
...
Here we observe that age and cholestoral have the highest correlations with SBP, and there are no high correlations within the predictor variables.
...
However, we can see that sex and cholestoral are not significant.
...
```{r}
Select<-c("ID","age","sex","chol","trestbps","fbs","thalach")
...
This study observed $n = 315$ patients who had a surgical procedure to remove a non-cancerous lesion of the lung, colon, breast, skin, ovary or uterus.  
...
betaplasma  | Plasma beta-carotene ($\text{ng}/\text{ml}$)
retplasma   | Plasma retinol ($\text{ng}/\text{ml}$)
...
```{r}
plasma <- read.csv("Plasma.csv",sep=",", header=TRUE, stringsAsFactors = TRUE)
str(plasma)
```
...
![RStudio layout](fig/04-rightskew.png)
